            ...
            <div className="border-t border-gray-200 bg-white p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                 <div className="w-full max-w-[1400px] mx-auto flex flex-col gap-4">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-stretch">
                        <div className={`flex-1 w-full p-4 rounded-lg border flex justify-between items-center ${calculations.effectiveGcashNet >= 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200' }`}><div className="flex flex-col justify-center"><div className={`text-xs font-bold uppercase mb-1 ${calculations.effectiveGcashNet >= 0 ? 'text-green-600' : 'text-red-400'}`}>Total Gcash NET</div><div className={`text-3xl font-extrabold ${calculations.effectiveGcashNet >= 0 ? 'text-green-700' : 'text-red-600'}`}>{calculations.effectiveGcashNet > 0 ? '+' : ''}₱{calculations.effectiveGcashNet.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div></div><div className="text-right flex items-center h-full">{Math.abs(calculations.effectiveGcashNet) < 1 ? (<span className="text-green-700 font-bold flex items-center gap-2 text-lg"><CheckCircle size={24}/> BALANCED</span>) : (<span className={`${calculations.effectiveGcashNet >= 0 ? 'text-green-600' : 'text-red-600'} font-bold flex items-center gap-2 text-lg`}><AlertTriangle size={24}/> {calculations.effectiveGcashNet < 0 ? 'SHORTAGE' : 'SURPLUS'}</span>)}</div></div>
